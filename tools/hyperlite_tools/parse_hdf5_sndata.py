...
"""Coarsen a FLASH-style HDF5 supernova snapshot (block AMR) to a
block-averaged Cartesian structure file plus diagnostic profile plots."""
...
# -- element/isotope to output-column index
...
def main(h5file):
    """Coarsen HDF5 supernova data and convert to ASCII.

    Averages each leaf block of H5FILE down to one zone, writes profile
    plots (rho_hdf5.png, speed_hdf5.png, temp_hdf5.png) and an
    input.allstr_x*y*z* structure file in the current directory.
    """
    ...
    for iblock in range(nblock):
        ...
        if f['node type'][iblock] == 1:
            ...
            vels.append(np.array([velx_min, velx_max, vely_min, vely_max,
                                  velz_min, velz_max]))
            # -- total block mass
            ...
            mass.append(mass_ib)
            dens.append(mass_ib / ((x_max - x_min) * (y_max - y_min)
                                   * (z_max - z_min)))
            ...
            for el_key, el_val in EL_DICT.items():
                mfrac[el_val] += dx * dy * dz * np.sum(
                    f['dens'][iblock, :, :, :] * f[el_key][iblock, :, :, :])
            mfrac /= mass_ib
            if np.sum(mfrac) != 1.0:
                print('sum(mfrac) = ', np.sum(mfrac), ' for iblock = ', iblock)
    ...
    if nx != np.size(np.unique(raw_str[:, 1])):
        print('Invalid nx')
    ny = np.size(np.unique(raw_str[:, 2]))
    if ny != np.size(np.unique(raw_str[:, 3])):
        print('Invalid ny')
    nz = np.size(np.unique(raw_str[:, 4]))
    if ny != np.size(np.unique(raw_str[:, 5])):
        print('Invalid nz')
    hd1 = 'cartesian\n'
    hd2 = ('  ' + str(nx) + '     ' + str(ny) + '     ' + str(nz) + '     '
           + str(15 + nelem) + '     ' + str(nelem) + '\n')
    hd3 = '   x_left'
    ...
    for lbl in expllabs:
        hd3 += lbl.rjust(12)
    for el_val in INV_EL_DICT.values():
        hd3 += el_val.rjust(12)
    hd = hd1 + hd2 + hd3
    # -- save
    np.savetxt('input.allstr_x' + str(nx) + 'y' + str(ny) + 'z' + str(nz),
               raw_str, fmt='% .4e', delimiter=' ', header=hd)
