...
Example:
>> stella2superlite -o path/to/sim-superlite/folder -p path/to/stella/res/folder -d 20 -t -s --show-plots
"""
...
from hyperlite_tools.myfuncts import (
    find_nearest_ind,
    hasNum,
    load_stella_prof,
    replace_line,
...
@click.command()
@click.option('-o', '--out-path', type=str, default=None,
              help='Absolute path to the folder to save input.str file '
              '[default: current directory]')
@click.option('-p', '--path', 'stella_profile_path', type=str, default=None,
              help='Absolute path to the Stella folder containing '
              'mesa.dayNNN_post_Lbol_max.data file '
              '[default: current directory]')
@click.option('-d', '--day', type=float, default=30, show_default=True,
...
@click.option('--tau-thresh', type=float, default=100, show_default=True,
              help='The Stella calculated optical depth at which the profile'
              ' is truncated')
@click.option('-s', '--sanity-check', is_flag=True,
              help='Check for consistency')
@click.option('-n', '--renorm', 'renormalize', is_flag=True,
              help='Whether to renormalize the mass fractions in each cell'
              ' if they do not add up to 1')
@click.option('--homologous-vel', is_flag=True,
              help='Create homologize the velocity profile')
@click.option('--inter', 'interpolate', is_flag=True,
              help='Interpolate the original data to a higher resolution'
              ' for 10> tau > 0.1')
@click.option('--merge', 'merge_inner', is_flag=True,
              help='Merge "n-merge" cells interior to tau=10 into 1 cell')
@click.option('--n-merge', type=int, default=5, show_default=True,
              help='Number of inner cells to merge into 1')
@click.option('-i', '--only-info', is_flag=True,
...
@click.option('--show-plots', 'show', is_flag=True,
              help='display the profile plots before saving')
@click.option('--out-prefix', type=str, default='Model', show_default=True,
              help='Common prefix for output profile plots')
@click.option('--template-dir', type=str, default=None,
              help='Directory containing the input.par.lte/.nlte template'
              ' decks [default: the w7 decks of a repository checkout]')
def main(out_path, stella_profile_path, day, truncate, tau_thresh,
         sanity_check, renormalize, homologous_vel, interpolate, merge_inner,
         n_merge, only_info, verbose, show, out_prefix, template_dir):
    """Convert a Stella profile file to a SuperLite input.str deck."""
    ...
    if out_path is None:
        out_path = os.getcwd()
    if stella_profile_path is None:
        stella_profile_path = os.getcwd()
    ...
    ### set values
    #-- NOTE: replicates the legacy script exactly, including using the
    #-- --merge *flag* (not --n-merge) here; the --inter/--merge path is
    #-- known-broken upstream (hard-coded factor-4 subdivision) and is
    #-- kept unchanged pending validation data
    nmerge = merge_inner
    ...
    dt_elem = [('h', float), ('he', float), ('c', float), ('n', float),
               ('o', float), ('ne', float), ('na', float), ('mg', float),
               ('si', float), ('s', float), ('ar', float), ('ca', float),
               ('ti', float), ('cr', float), ('fe', float), ('co', float),
               ('ni', float), ('cr48', float), ('fe52', float),
    ...
    for name in abund.dtype.names:
        if not (name == 'ni56' or name == 'co56' or name == 'fe52'
                or name == 'cr48'):
    ...
    dt_slite = [('x_left', float), ('x_right', float), ('vx_left', float),
                ('vx_right', float), ('mass', float), ('rad_temp', float),
                ('temp', float), ('h', float), ('he', float), ('c', float),
                ('n', float), ('o', float), ('ne', float), ('na', float),
                ('mg', float), ('si', float), ('s', float), ('ar', float),
                ('ca', float), ('ti', float), ('cr', float), ('fe', float),
                ('co', float), ('ni', float)]
    ...
    if interpolate:
        if merge_inner:
            ...
            outdata['mass'] = np.concatenate((mass_remain, mass_interp,
                                              mass_outer))
            outdata['rad_temp'] = np.concatenate((radtemp_remain, radtemp_interp,
                                                  radtemp_outer))
            outdata['temp'] = np.concatenate((temp_remain, temp_interp,
                                              temp_outer))
            for name in abund.dtype.names:
                if abund[name].any():
